...
class Zombie:
  ...
  def __str__(self):
      return f"Strength:{self.strength} Speed:{self.speed}"
  ...
  def spawn(cls):
    ...
    while count < new_zombies:
      ...
      Zombie.horde.append(Zombie(strength, speed))
      count += 1
  ...
  def increase_plague_level(cls):
    day_plague = random.randint(0, 2)
    Zombie.plague_level += day_plague
  ...
  def fight(self):
    """represents what happens when you try to fight a zombie."""
